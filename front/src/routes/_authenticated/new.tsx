...
function NewPostPage() {
  ...
  // Sync anonymous default from user profile once it loads
  useEffect(() => {
    if (profile) setAnonymous(profile.anonymous_default);
  }, [profile]);
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (!authorId) {
      // Should not hit this because route is authenticated, but fallback just in case
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Mobile tab switcher */}
      <div className="lg:hidden glass rounded-2xl p-1 flex gap-1">
        {(["write", "preview"] as const).map((t) => (
          <button
            ...
            className={`flex-1 py-2 rounded-xl text-sm font-medium transition ${
              mobileTab === t ? "bg-white/10 text-white" : "text-white/55 hover:text-white"
            }`}
          >
